...
So far, we have only discussed the pure Yang-Mills theory.
However, a renormalizable quantum field theory in four dimensions could also contain scalar fields with helicity $h=0$.
Thus, we can consider adding extra scalar or fermionic terms to the Yang-Mills Lagrangian.
\begin{intu}
    Irrespective of the details of the extra content and couplings of the fermionic and scalar matter fields in a gauge theory, the $n$-gluon tree level amplitudes are identical to the pure Yang-Mills theory ones. 
    This is because in a tree-level diagram with only external gluon legs, scalars or fermions cannot appear: 
    they are always produced in pairs from gluon lines and thus have to exit the diagram at tree-level\cite{Henn:2014yza}.
\end{intu}
So one can assemble the matter content of the gauge theory according to one's preference.
It can be shown that \textit{maximally supersymmetric} Yang-Mills theory or $\mathcal{N}=4$ super Yang-Mills theory has remarkable properties.
